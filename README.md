...
총괄 메뉴의 **QR 인쇄**(`/teacher/:eventId/qr`)에서 A4로 인쇄합니다. QR은 브라우저 안에서 만들며(무료 MIT 라이브러리 `qrcode-generator`) 외부 서비스에 주소를 보내지 않습니다.

- 미션 교실 도착 QR 5장: 교실마다 한 장, 교실 입구에 붙입니다. 팀 기기로 찍으면 그 기기에 입장한 팀의 도착이 기록됩니다. 같은 QR을 여러 번 찍어도 기록은 하나입니다.
- 팀 입장 QR: 반마다 한 장(다섯 팀), 점선을 따라 잘라 팀에 나눠 줍니다. 학년과 학급을 골라 인쇄합니다.
- "QR에 넣을 사이트 주소"는 지금 열어 둔 주소로 채워집니다. 내 컴퓨터(localhost)에서 열었다면 배포한 주소로 바꿔야 학생 기기로 열립니다(화면에서 알려 줍니다).
- 인쇄 창에서는 A4 세로, 배율 100%, "머리글과 바닥글" 끄기를 권합니다. 인쇄한 뒤 디벗 카메라로 한 장 찍어 입장할 때 쓴 것과 같은 브라우저로 열리는지 먼저 확인하세요.
- 인쇄물이 없을 때는 부스 화면의 "교실 QR 보기"에서 QR을 화면에 띄워 보여 줄 수 있습니다.

### 기기 잠금 해제

디벗이 다른 팀으로 잘못 입장하면 학생 화면에 지금 묶여 있는 팀과 **기기 번호**(네 글자)가 나옵니다. 교사는 그 반의 학급 화면(학급 카드 → 학급) 아래 **팀 기기 잠금 해제**에서 "입장한 기기 보기"를 누르고, 같은 기기 번호 줄의 "잠금 해제"를 누릅니다. 그 기기는 다시 팀을 골라 입장할 수 있고, 팀이 낸 제출·카드·도착 기록은 그대로 남습니다. 기기 목록은 버튼을 눌렀을 때만 읽습니다.

## 실제 문항 넣기

미션 내용과 최종 미션 문제는 `content/작성/`의 양식에 적은 뒤 Claude Code에 적용을 요청합니다. 적는 방법과 주의할 점은 [content/README.md](content/README.md)에 있습니다. `content/작성/`은 정답이 들어 있어 GitHub에 올라가지 않으니 백업은 직접 해 주세요.
