/*
 * Material Symbols Rounded(가중치 400) SVG를 빌드에 포함한다.
 * 외부 폰트·CDN 없이 학교 네트워크가 끊겨도 아이콘이 보인다.
 * 새 아이콘은 이 목록에 한 줄씩 추가한다.
 */
